 ...
 * => 비밀번호 제공 없이, (네이브, 구글, 카카오 등의)일부 기능만 사용할 수 있는 임시 토큰을 발급해주는 방식
 *
 * === OAuth2 사용 흐름 ===
 * 1) 프론트에서 "소셜 계정 로그인"버튼클릭
 *  : 브라우저가 백엔드 /oauth2/authorization/{sns-name} 으로 이동
 *
 * 2) Spring Security가 자동으로 소셜 로그인 페이지러 리다이렉트
 *
 * 3) 사용자가 소셜 로그인 완료
 *  : 소셜 계정이 우리 서버의 /login/oauth2/code/{sns-name}으로 코드를 보냄
 *
 * 4) 서버 쪽 CustomOAuth2UserService가 코드를 AccessToke 으로 교환하고
 *      , 사용자 정보(이메일/닉네임 등)를 가져옴
 *
 * 5) 우리 DB에 해당 이메일의 유저가 있으면 -> 로그인
 *                               없으면 -> 자동 회원가입 후 로그인
 *
 * 6) 로그인에 성공하면 Oauth24AuthenticationSuccessHandler 에서
